...
public class GetDataTask {
    ...
    @Value("${task.getData.baseUrl}")
    private String baseUrl;

    @Value("${task.getData.enable}")
    private Boolean getDataEnalbe;

    @Autowired
    private ICourseTypeService courseTypeService;

    private static Logger logger = LoggerFactory.getLogger(GetDataTask.class);

    @Scheduled(cron = "${task.getData.cron}")
    public void getData(){
        if (getDataEnalbe){
